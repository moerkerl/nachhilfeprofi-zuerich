...
const GA_ADS_ID = 'AW-16764247259'
const CONVERSION_LABEL = 'uZYNCJ6QhIkbENu56Lk-'
...
export default function ConversionTracker() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && window.sessionStorage.getItem('nachhilfeprofi_conversion_tracked')) {
    ...
    timer.current = setTimeout(() => {
      if (typeof window !== 'undefined' && typeof window.gtag === 'function') {
        ...
        const transactionId = `nachhilfeprofi_lead_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        
        // Double-check that conversion hasn't been sent yet
        if (window.sessionStorage.getItem('nachhilfeprofi_conversion_tracked')) {
        ...
        window.sessionStorage.setItem('nachhilfeprofi_conversion_tracked', 'true');
